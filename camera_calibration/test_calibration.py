...
import os
#9:7 then 7x5 will be detected

#Config Variables - Enter their values according to your Checkerboard
no_of_columns = 7   #number of columns of your Checkerboard
no_of_rows = 9  #number of rows of your Checkerboard
square_size = 24.0 # size of square on the Checkerboard in mm
debug_images = False
folder_path = 'images'
...
class MyGUI():
	...
	def calculateIntrinsicParam(self, b):
		...
		if not os.path.exists('../calib_output_test'):
			os.mkdir('../calib_output_test')    #make output folder if not exists
		with open('../calib_output_test/intrinsic.txt', 'w+') as f:
			f.write('A=\n{}'.format(A))
	...
	def calculateExtrinsicParam(self, A):
		...
		if not os.path.exists('../calib_output_test'):
			os.mkdir('../calib_output_test')    #make output folder if not exists
		with open('../calib_output_test/extrinsic.txt', 'w+') as f:
			f.write('[R|t]=\n{}'.format(Rt))
		return Rt
	...
	def predict2Dpoints(self, points3D, A, Rt, lamda):
		...
		if not os.path.exists('../calib_output_test'):
            		os.mkdir('../calib_output_test')    #make output folder if not exists
		f = open('../calib_output_test/predicted VS actual.txt', 'w+')
